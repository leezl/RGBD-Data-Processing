...
class TaredPickler:
    ...
    def set_up(self, dataDirs):
        ...
        print "Is this a list? ",type(self.list_dictionary)
    ...
    def load_dictionary(self, fileList):
        ...
        for filepath, filename, length in fileList:
            ...
            print "Adding to dictionary: ",name
            assert type(name) == type("string"), "Adding a non-string to dictionary "+str(name)
            assert name[0] not in '0123456789', "The name is a number not a word "+str(name)
        ...
        for key in total_dictionary:
            print key,',',total_dictionary[key]
            print type(key),',',type(total_dictionary[key])
    ...
    def reflect_image(self, name, coords, image, depth):
        #self.display_color(image)
        #self.display_color(image[coords[0]:coords[2],coords[1]:coords[3],:3])
        #reflect image
        imageRef = image[:,::-1,:]
        #reflect mask/coords
        #maskRef = mask[::-1,:,:]
        #coordsRef = ( image.shape[1]-coords[2], coords[1], image.shape[1]-coords[0], coords[3])
        coordsRef = (coords[0], image.shape[1]-coords[3], coords[2], image.shape[1]-coords[1])
        #self.display_color(imageRef)
        #self.display_color(imageRef[coordsRef[0]:coordsRef[2],coordsRef[1]:coordsRef[3],:3])
        #reflect depth
        depthRef = depth[:,::-1]
        #return...or just add...?
        ...
        for index in range(otherI, len(self.fileList)):
            ...
            if self.do_not_exit:
                ...
                if (self.i+1)%self.batchSize == 0:
                    ...
                    #set outputfile
                    filenum = ((self.i+1)/self.batchSize)-1
                    print "Pickling Batch: ",self.outputLoc+"/data_batch_"+str(((self.i+1)/self.batchSize)-1)
                    if filenum>0:
                        assert os.path.exists(self.outputLoc+"/data_batch_"+str(filenum-1)), \
                            "Previous batch does not exist, "+str(filenum-1)+','+str(self.batchSize)+','+str(self.i)
                    outputFileName = self.outputLoc+"/data_batch_"+str(((self.i+1)/self.batchSize)-1)
                    if not os.path.exists(self.outputLoc):
                        os.makedirs(self.outputLoc)
                    outputFile = gzip.open(outputFileName, 'wb')
                    assert len(self.data)==len(self.labels), "Data and labels are different lengths: failed batch"
                    assert len(self.data)==len(self.crop_list), "Data and crop_boxes and different lengths: failed batch"
                    for item in self.labels:
                        assert type(item) == type(1), "Adding a non-integer as label "+str(item)
                    #pickle
