...
*/, &paths, &count);
        ...
        // Init motor
        res = xnUSBSendControl(m_dev, /*(XnUSBControlType)0xc0*/XN_USB_CONTROL_TYPE_VENDOR, 0x10, 0x00, 0x00, buf, sizeof(buf), 0);
        if (res != XN_STATUS_OK) {
                xnPrintError(res, "xnUSBSendControl failed (first)");
                Close();
                return false;
        }
        ...
        if (res != XN_STATUS_OK) {
                xnPrintError(res, "xnUSBSendControl failed (second)");
                Close();
                return false;
        }
        return true;
